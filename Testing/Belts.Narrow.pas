...
{$ENDIF}
  ...
  V_max := 30; { Максимально допустимая скорость ремня }
...
begin
  if Count > 0 then
    ...
    raise EListError.Create
      ('[TNarrowVeeBelt.First] Попытка выбрать первую передачу в пустой коллекции');
end;
...
begin
  if Index < Count then
    ...
    raise EListError.Create
      ('[TNarrowVeeBelt.GetGears] Попытка обратиться к передаче с номером больше максимального');
end;
...
begin
  ...
  if SectionIndex <= High(UclinT) then
    ...
    raise EAccessViolation.Create
      ('[TNarrowVeeBelt.GetSection] Некорректно задан номер сечения (SectionIndex)');
end;
...
begin
  if Count > 0 then
    ...
    raise EListError.Create
      ('[TNarrowVeeBelt.Last] Попытка выбрать последнюю передачу в пустой коллекции');
end;
...
begin
  if n1 = 0 then
    raise EZeroDivide.CreateFmt
      ('[TNarrowVeeBelt.MinDiameter] Ошибка деления на ноль при расчете минимально допустимого диаметра шкива (D1). Частота вращения ведущего шкива (n1) не может быть равна нулю',
      []);
  ...
  begin
    ...
    if Tip_ukl <= High(UclinT) then
      ...
      raise ERangeError.CreateFmt
        ('[TNarrowVeeBelt.MinDiameter] При попытке рассчитать минимально допустимый диаметр шкива (D1) неправильно выбирается сечение (Tip_ukl=%d)',
        [Tip_ukl]);
  end
  else
...
begin
  if n1 = 0 then
    raise EZeroDivide.Create
      ('[TNarrowVeeBelt.SetD1] Ошибка деления на ноль при записи диаметра шкива (D1), частота вращения (n1) не должна быть равна нулю');
  ...
  begin
    ...
    if Tip_ukl > High(UclinT) then
      raise ERangeError.CreateFmt
        ('[TNarrowVeeBelt.SetD1] При попытке рассчитать минимально допустимый диаметр шкива (D1) неправильно выбирается сечение (Tip_ukl=%d)',
        [Tip_ukl]);
  end
  else
  ...
  if (Value < MinDiameter) and (Value <> 0) then
    raise ERangeError.CreateFmt
      ('[TNarrowVeeBelt.SetD1] Диаметр шкива (D1) не может быть равен %g, минимально допустимое значение для ремня %s равно %g мм (ГОСТ 20889-88)',
...
begin
  if Index < Count then
    ...
    raise EListError.Create
      ('[TNarrowVeeBelt.SetGears] Попытка записать данные в передачу с номером больше максимального');
end;
...
begin
  ...
  try
    ...
    on E: EZeroDivide do
      raise EZeroDivide.Create
        ('[TNarrowVeeBelt.Calculate] Ошибка деления на ноль при расчёте диаметра ведомого шкива (d2), передаточное отношение (up) не может быть равно нулю');
    else
      raise Exception.Create
        ('[TNarrowVeeBelt.Calculate] Ошибка при расчёте диаметра ведомого шкива (d2)');
  end;
  ...
  try
    ...
    on E: Exception do
      raise Exception.Create
        ('[TNarrowVeeBelt.Calculate] Не удалось округлить диаметр ведомого шкива (d2) до ближайшего значения из ряда линейных размеров');
  end;
  ...
  try
    ...
    on E: Exception do
      raise Exception.Create
        ('[TNarrowVeeBelt.Calculate] Не удалось округлить длину ремня (L) до ближайшего допустимого значения из массива длин клиновых ремней');
  end;
  ...
  try
    ...
    on E: Exception do
      raise Exception.Create
        ('[TNarrowVeeBelt.Calculate] Ошибка при расчёте межосевого расстояния (a). Возможно, неправильно рассчиталась длина ремня (L)');
  end;
  ...
  try
    ...
    on E: EZeroDivide do
      raise EZeroDivide.Create
        ('[TNarrowVeeBelt.Calculate] Ошибка деления на ноль при расчёте угла обхвата ремня (alfa), уточнённое межосевое расстояние (a) оказалось равно нулю');
    else
      raise Exception.Create
        ('[TNarrowVeeBelt.Calculate] Ошибка при расчёте угла обхвата ремня (alfa)');
  end;
  ...
  try
    ...
    on E: EZeroDivide do
      raise EZeroDivide.Create
        ('[TNarrowVeeBelt.Calculate] Ошибка деления на ноль при расчёте пробега ремня (nyu), длина ремня (L) оказалась равна нулю');
    else
      raise Exception.Create
        ('[TNarrowVeeBelt.Calculate] Ошибка при расчёте пробега ремня (nyu)');
  end;
  ...
  try
    ...
    on E: Exception do
      raise EAccessViolation.Create
        ('[TNarrowVeeBelt.Calculate] Подбор значений из таблиц вызвал ошибку, неверно задан номер сечения (SectionIndex)');
  end;
  ...
  try
    ...
    on E: EZeroDivide do
      raise EZeroDivide.Create
        ('[TNarrowVeeBelt.Calculate] Ошибка деления на ноль при расчёте окружного усилия (Ft), скорость ремня (V) оказалась равна нулю');
    else
      raise Exception.Create
        ('[TNarrowVeeBelt.Calculate] Ошибка при расчёте окружного усилия (Ft)');
  end;
  ...
  try
    ...
    on E: Exception do
      raise Exception.Create
        ('[TNarrowVeeBelt.Calculate] Ошибка при расчёте (Sigma_t)');
  end;
  ...
  try
    ...
    on E: EZeroDivide do
      raise EZeroDivide.Create
        ('[TNarrowVeeBelt.Calculate] Ошибка деления на ноль при расчёте числа ремней (z)');
    else
      raise Exception.Create
        ('[TNarrowVeeBelt.Calculate] Ошибка при расчёте числа ремней (z)');
  end;
  ...
  try
    ...
    on E: EInvalidOp do
      raise EInvalidOp.Create
        ('[TNarrowVeeBelt.Calculate] Взят корень из отрицательного значения при расчёте давления на валы ремня (Fb)');
    else
      raise Exception.Create
        ('[TNarrowVeeBelt.Calculate] Ошибка при расчёте давления на валы ремня (Fb)');
  end;
  ...
  try
    ...
    on E: EZeroDivide do
      raise EZeroDivide.Create
        ('[TNarrowVeeBelt.Calculate] Ошибка деления на ноль при расчёте долговечности (Lh)');
    on E: EInvalidOp do
      raise EInvalidOp.Create
        ('[TNarrowVeeBelt.Calculate] Под логарифмом оказалось отрицательное или нулевое значение при расчёте долговечности (Lh)');
    else
      raise Exception.Create
        ('[TNarrowVeeBelt.Calculate] Ошибка при расчёте долговечности (Lh)');
  end;

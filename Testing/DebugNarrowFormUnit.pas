...
procedure TDebugNarrowForm.ApplicationEvents1Exception(Sender: TObject;
  E: Exception);
var
...
procedure TDebugNarrowForm.Button1Click(Sender: TObject);
var
  I: Integer;
begin
...
procedure TDebugNarrowForm.Button2Click(Sender: TObject);
var
  I: Integer;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          res := (a.Gears[I].z);
          re := (a.Gears[I].SectionIndex);
          Writeln(F, FloatToStr(a.N1) + #9 + FloatToStr(Lh) + (FloatToStr(re)));
...
procedure TDebugNarrowForm.Button3Click(Sender: TObject);
begin
...
procedure TDebugNarrowForm.Button4Click(Sender: TObject);
var
  I: Integer;
begin
...
procedure TDebugNarrowForm.ButtonMonkeyClick(Sender: TObject);
var
  I: Integer;
  F: TextFile;
begin
...
procedure TDebugNarrowForm.ComboBoxCpChange(Sender: TObject);
begin
  case ComboBoxCp.ItemIndex of
    0:
      a.Cp := 1;
...
procedure TDebugNarrowForm.ComboBoxD1Change(Sender: TObject);
begin
  if ComboBoxD1.ItemIndex = 0 then
    a.d1 := 0
  else
...
procedure TDebugNarrowForm.ComboBoxD1KeyPress(Sender: TObject; var Key: Char);
begin
  if Key <> #13 then
    Key := #0;
end;

procedure TDebugNarrowForm.ComboBoxSectChange(Sender: TObject);
begin
  a.SectionIndex := ComboBoxSect.ItemIndex;
  Refresh;
end;

procedure TDebugNarrowForm.ComboBoxTensChange(Sender: TObject);
begin
  case ComboBoxTens.ItemIndex of
    0:
      a.Tension := ttAutomatic;
...
procedure TDebugNarrowForm.EditN1Exit(Sender: TObject);
begin
  try
    a.N1 := StrToFloat(EditN1.Text);
  except
...
procedure TDebugNarrowForm.EditP1Exit(Sender: TObject);
begin
  try
    a.P1 := StrToFloat(EditP1.Text);
  except
...
procedure TDebugNarrowForm.EditUpExit(Sender: TObject);
begin
  try
    a.UP := StrToFloat(EditUp.Text);
  except
...
procedure TDebugNarrowForm.ComboBoxAExit(Sender: TObject);
begin
  try
    if ComboBoxA.Text = 'Не важно' then
      a.a := 0
...
procedure TDebugNarrowForm.FormShow(Sender: TObject);
var
  NB: TNarrowVeeInput;
begin
...
procedure TDebugNarrowForm.Refresh;
var

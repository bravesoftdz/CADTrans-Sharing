...
type
  ...
  // Входные параметры для клиновой ремённой передачи
  TVeeInput = record
    P1,             {Мощность на ведущем валу, кВт}
    n1,             {Частота вращения ведущего шкива, об/мин}
    up,             {Передаточное число}
    ...
    Tension: TTensionType;
                    {Тип натяжения ремня; автоматическое-периодическое}
    SectionIndex: integer;
                    {Номер типоразмера ремня; 0 - Любой, 1 - О, 2 - А, 3 - Б, 4 - В, 5 - Г, 6 - Д, 7 - Е}
  end;

  // Выходные параметры для одного экземпляра клиновой ремённой передачи
  TVeeOutput = record
    d1,             {Диаметр меньшего (ведущего) шкива, мм}
    L,              {Длина ремня, мм}
    d2,             {Диаметр большего (ведомого) шкива, мм}
    a: extended;    {Межосевое расстояние, мм}
    SectionIndex: integer;
                    {Номер типоразмера ремня; 1 - О, 2 - А, 3 - Б, 4 - В, 5 - Г, 6 - Д, 7 - Е}
    z: integer;     {Число ремней}
    d_e1,           {Наружный диаметр ведущего шкива, мм}
    d_e2,           {Наружный диаметр ведомого шкива, мм}
    d_f1,           {Диаметр по дну впадин ведущего шкива, мм}
  ...
  // Коллекция выходных параметров клиновой ремённой передачи
  TVeeOutputs = array of TVeeOutput;

  // Входные параметры для плёночной ремённой передачи
  TFilmInput = record
    P1,             {Мощность на ведущем валу, кВт}
    n1,             {Частота вращения ведущего шкива, об/мин}
    up,             {Передаточное число}
    ...
    CordTypeIndex: integer;
                    {Номер толщины корда ремня}
                    {0 - Любой, 1 - 0.4, 2 - 0.6}
                    {3 -   0.8, 4 - 1.0, 5 - 1.2}
  end;

  // Выходные параметры для одного экземпляра плёночной ремённой передачи
  TFilmOutput = record
    d1,             {Диаметр меньшего (ведущего) шкива, мм}
    L,              {Длина ремня, мм}
    d2,             {Диаметр большего (ведомого) шкива, мм}
  ...
  // Коллекция выходных параметров плёночной ремённой передачи
  TFilmOutputs = array of TFilmOutput;

// Входные параметры для плоскоремённой передачи
  TFlatInput = record
    P1,             {Мощность на ведущем валу, кВт}
    n1,             {Частота вращения ведущего шкива, об/мин}
    up,             {Передаточное число}
    ...
    Cp: extended;   {Коэффициент режима работы}
                    ...
                    {1.25 - значительные колебания, 1.55 - ударная нагрузка}
    z: integer;
                    {Число слоёв корд-ткани; 0..6, где 0 - Любое}
    Tension: TTensionType;
                    {Тип натяжения ремня; автоматическое-периодическое}
    CordTypeIndex: integer;
                    {Номер типа корда ремня}
                    {0 - Любой,    1 - Б-800,  2 - ТА-150, 3 - ТК-150}
                    {4 - ТК-200-2, 5 - ТА-300, 6 - ТК-300}
  end;

  // Выходные параметры для одного экземпляра плоскоремённой передачи
  TFlatOutput = record
    d1,             {Диаметр меньшего (ведущего) шкива, мм}
    ...
    CordTypeIndex: integer;
                    {Номер типа корда ремня}
                    {1 - Б-800,    2 - ТА-150, 3 - ТК-150}
                    {4 - ТК-200-2, 5 - ТА-300, 6 - ТК-300}
    z: integer;     {Число слоёв корд-ткани}
    b: extended;    {Ширина ремня, мм}
    Bs: extended;   {Ширина шкива, мм}
    F0,             {Усилие предварительного натяжения, Н}
    Fb,             {Давление на валы от натяжения ремня, Н}
  ...
  // Коллекция выходных параметров плоскоремённой передачи
  TFlatOutputs = array of TFlatOutput;

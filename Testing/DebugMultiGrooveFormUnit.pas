...
uses
  ...
  Vcl.ComCtrls, Vcl.ExtCtrls, Belts.MultiGroove, Vcl.AppEvnts;
// Windows,Messages,SysUtils,Variants,Classes,Graphics,
// Controls,Forms,Dialogs,StdCtrls,Grids,
// ComCtrls,ExtCtrls, Belts.Narrow, AppEvnts;

type
  TDebugMultiGrooveForm = class(TForm)
...
procedure TDebugMultiGrooveForm.ApplicationEvents1Exception(Sender: TObject;
  E: Exception);
var
...
begin
  ...
  try
    ...
    Writeln(LogFile, DateTimeToStr(Now) + ' ' + E.Message + #13 + ' P1=' +
      ...
      ' D1=' + FloatToStr(a.Gears[a.Count - 1].d1) + ' A=' +
      FloatToStr(a.Gears[a.Count - 1].a) + ' Cp=' + FloatToStr(a.Cp) + ' Sect='
      + FloatToStr(a.Gears[a.Count - 1].SectionIndex));
    // показать ошибку
...
procedure TDebugMultiGrooveForm.Button1Click(Sender: TObject);
var
  I: Integer;
begin
...
procedure TDebugMultiGrooveForm.Button2Click(Sender: TObject);
var
  I: Integer;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Writeln(F, FloatToStr(a.N1) + #9 + FloatToStr(Lh) + #9 +
            (FloatToStr(re)));
...
procedure TDebugMultiGrooveForm.Button3Click(Sender: TObject);
begin
...
procedure TDebugMultiGrooveForm.Button4Click(Sender: TObject);
var
  I: Integer;
begin
...
procedure TDebugMultiGrooveForm.ButtonMonkeyClick(Sender: TObject);
var
  I: Integer;
  F: TextFile;
begin
...
procedure TDebugMultiGrooveForm.ComboBoxCpChange(Sender: TObject);
begin
  case ComboBoxCp.ItemIndex of
    0:
      a.Cp := 1;
...
procedure TDebugMultiGrooveForm.ComboBoxD1Change(Sender: TObject);
begin
  if ComboBoxD1.ItemIndex = 0 then
    a.d1 := 0
  else
...
procedure TDebugMultiGrooveForm.ComboBoxD1KeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key <> #13 then
    Key := #0;
...
procedure TDebugMultiGrooveForm.ComboBoxSectChange(Sender: TObject);
begin
  a.SectionIndex := ComboBoxSect.ItemIndex;
  Refresh;
end;

procedure TDebugMultiGrooveForm.ComboBoxTensChange(Sender: TObject);
begin
  case ComboBoxTens.ItemIndex of
    0:
      a.Tension := ttAutomatic;
...
procedure TDebugMultiGrooveForm.EditN1Exit(Sender: TObject);
begin
  try
    a.N1 := StrToFloat(EditN1.Text);
  except
...
procedure TDebugMultiGrooveForm.EditP1Exit(Sender: TObject);
begin
  try
    a.P1 := StrToFloat(EditP1.Text);
  except
...
procedure TDebugMultiGrooveForm.EditUpExit(Sender: TObject);
begin
  try
    a.UP := StrToFloat(EditUp.Text);
  except
...
procedure TDebugMultiGrooveForm.ComboBoxAExit(Sender: TObject);
begin
  try
    if ComboBoxA.Text = 'Не важно' then
      a.a := 0
...
procedure TDebugMultiGrooveForm.FormShow(Sender: TObject);
var
  NB: TMultiGrooveInput;
begin
  ...
  a := TMultiGrooveBelt.Create(NB);

  Refresh;
end;

procedure TDebugMultiGrooveForm.Refresh;
var

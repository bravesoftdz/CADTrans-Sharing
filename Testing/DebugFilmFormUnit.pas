...
uses
  ...
  Vcl.ComCtrls, Vcl.ExtCtrls, Belts.Film, Vcl.AppEvnts;
// Windows,Messages,SysUtils,Variants,Classes,Graphics,
// Controls,Forms,Dialogs,StdCtrls,Grids,
// ComCtrls,ExtCtrls, Belts.Film, AppEvnts;

type
  TDebugFilmForm = class(TForm)
...
procedure TDebugFilmForm.ApplicationEvents1Exception(Sender: TObject;
  E: Exception);
var
...
procedure TDebugFilmForm.Button1Click(Sender: TObject);
var
  I: Integer;
begin
  ...
  a.CordTypeIndex := ComboBoxSect.ItemIndex;
  ...
  begin
    ...
    begin
      ...
      StringGrid1.Cells[5, I + 1] := a.CordBeltType(a.Gears[I].CordTypeIndex);
...
procedure TDebugFilmForm.Button2Click(Sender: TObject);
var
  I: Integer;
  cnt, resz, zprev : Integer;
  res,secprev,re,lh:extended;
  F: TextFile;
begin
  AssignFile(F, 'ResultP1.txt');
  Rewrite(F);
  resz := 1;
  cnt := 0;
  // a.P1 := 10;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if ((a.Gears[I].CordTypeIndex) > secprev) then
        begin
          lh:=(a.Gears[I].Lh);
          secprev :=(a.Gears[I].CordTypeIndex);
          re := (a.Gears[I].CordTypeIndex);
          Writeln(F, FloatToStr(a.P1) + #9+FloatToStr(lh)+ #9+ (FloatToStr(re)));
...
procedure TDebugFilmForm.Button3Click(Sender: TObject);
begin
...
procedure TDebugFilmForm.Button4Click(Sender: TObject);
var
  I: Integer;
begin
  ...
  begin
    ...
    sleep(100); // ждём 100 миллисекунд
...
procedure TDebugFilmForm.ButtonMonkeyClick(Sender: TObject);
var
  I: Integer;
  F: TextFile;
begin
  for I := 0 to 1000 do // задаём количество повторов
  begin
    Refresh; // заполняем выпадающие списки D1 и А значениями из ГОСТ
    try
...
procedure TDebugFilmForm.ComboBoxCpChange(Sender: TObject);
begin
  case ComboBoxCp.ItemIndex of
    0:
      a.Cp := 1;
...
procedure TDebugFilmForm.ComboBoxD1Change(Sender: TObject);
begin
  if ComboBoxD1.ItemIndex = 0 then
    a.d1 := 0
  else
...
procedure TDebugFilmForm.ComboBoxD1KeyPress(Sender: TObject; var Key: Char);
begin
  if Key <> #13 then
    Key := #0;
end;

procedure TDebugFilmForm.ComboBoxSectChange(Sender: TObject);
begin
  a.SectionIndex := ComboBoxSect.ItemIndex;
  Refresh;
end;

procedure TDebugFilmForm.ComboBoxTensChange(Sender: TObject);
begin
  case ComboBoxTens.ItemIndex of
    0:
      a.Tension := ttAutomatic;
...
procedure TDebugFilmForm.EditN1Exit(Sender: TObject);
begin
  try
    a.N1 := StrToFloat(EditN1.Text);
  except
...
procedure TDebugFilmForm.EditP1Exit(Sender: TObject);
begin
  try
    a.P1 := StrToFloat(EditP1.Text);
  except
...
procedure TDebugFilmForm.EditUpExit(Sender: TObject);
begin
  try
    a.UP := StrToFloat(EditUp.Text);
  except
...
procedure TDebugFilmForm.ComboBoxAExit(Sender: TObject);
begin
  try
    if ComboBoxA.Text = 'Не важно' then
      a.a := 0
...
procedure TDebugFilmForm.FormShow(Sender: TObject);
var
  NB: TFilmInput;
begin
  ...
  a := TFilmBelt.Create(NB);

  Refresh;
end;

procedure TDebugFilmForm.Refresh;
var

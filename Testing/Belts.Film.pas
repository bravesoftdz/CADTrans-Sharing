...
Var
  ...
  b_max, b_min, p0, C_Q, Sigma_e, Sdelta, L1: extended;
...
begin
  ...
  if a = 0 then
    Output.a := 1.05 * (Output.d1 + Output.d2);
  ...
  try
    RoundGOST(TArrayLP, 1, Round(Array_plen[Output.CordTypeIndex, 7]),
      Output.L, L1, id2);
  except
    on E: Exception do
      raise Exception.Create
  ...
  try
    if (sqr(Output.L - Pi * (Output.d1 + Output.d2) / 2) - 2 *
      sqr(Output.d1 - Output.d2) > 0) then
    begin
      Output.L := L1;
    ...
    on E: Exception do
      raise Exception.Create
        ('[TFilmBelt.Calculate] Ошибка при расчёте межосевого расстояния (a). Возможно, неправильно рассчиталась длина ремня (L) '+e.ClassName);
  end;
  ...
  try
    ...
    b_min := TArrayBP[Round(Array_plen[VarCordType, 3])];
  ...
  try
    if sqr(f1) + sqr(F2) - 2 * f1 * F2 * cos((180 - Output.alfa) * Pi / 180) > 0
    then
  ...
  try
    ...
    if (Sdelta * Output.b * (qw - 1) <> 0) and (Output.d1 <> 0) then
      Sigma_e := 0.1 * qw * Ft / (Sdelta * Output.b * (qw - 1)) + 90 * Sdelta /
        Output.d1 + 1.2E-3 * sqr(Output.V)
    else
      Sigma_e := -1;

    if (Sigma_e > 0) and (Output.V * exp(4 * ln(Sigma_e)) <> 0) then
...
begin
  ...
  begin
    ...
    until CurrGear.L >= TArrayLP[Round(Array_plen[CordTypeIndex, 7])];
  end;
  ...
  begin
    ...
    begin
      { TODO : Проверить что выдается диаметр }
      d1x := TArrayD[Round(Array_plen[CordIndex, 2])];
      repeat

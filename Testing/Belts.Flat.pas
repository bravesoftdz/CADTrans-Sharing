...
begin
  ...
  if CordTypeIndex <= High(FlatT) then
    ...
    raise EAccessViolation.Create
      ('[TFlatBelt.CordBeltType] Некорректно задан номер корд-ткани (CordTypeIndex)');
end;
  ...
  try
    ...
    on E: EZeroDivide do
      raise EZeroDivide.Create
        ('[TFlatBelt.MinDiameter] ООшибка деления на ноль при расчете минимально допустимого диаметра шкива (D1). Частота вращения ведущего шкива (n1) оказалась равна нулю');
    else
      raise Exception.Create
        ('[TFlatBelt.MinDiameter] Ошибка при расчёте минимально допустимого диаметра шкива (D1)');
  end;
end;
...
begin
  ...
  if (Value < MinDiameter) and (Value <> 0) then
    raise ERangeError.CreateFmt
      ('[TFlatBelt.SetD1] Диаметр шкива (D1) не может быть равен %g, минимально допустимое значение для корд-ткани %s, равно %g мм',

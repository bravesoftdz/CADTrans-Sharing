...
uses
  ...
  Vcl.ComCtrls, Vcl.ExtCtrls, Belts.Flat, Vcl.AppEvnts;
// Windows,Messages,SysUtils,Variants,Classes,Graphics,
// Controls,Forms,Dialogs,StdCtrls,Grids,
// ComCtrls,ExtCtrls, Belts.Flat, AppEvnts;

type
  TDebugFlatForm = class(TForm)
...
procedure TDebugFlatForm.ApplicationEvents1Exception(Sender: TObject;
  E: Exception);
var
...
procedure TDebugFlatForm.Button1Click(Sender: TObject);
var
  I: Integer;
begin
  ...
  begin
    ...
    begin
      ...
      StringGrid1.Cells[5, I + 1] := a.Section(a.Gears[I].CordTypeIndex);
...
procedure TDebugFlatForm.Button2Click(Sender: TObject);
var
  I: Integer;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Writeln(F, FloatToStr(a.UP) + #9 + FloatToStr(res) + #9 +
            (FloatToStr(re)));
...
procedure TDebugFlatForm.Button3Click(Sender: TObject);
begin
...
procedure TDebugFlatForm.Button4Click(Sender: TObject);
var
  I: Integer;
begin
...
procedure TDebugFlatForm.ButtonMonkeyClick(Sender: TObject);
var
  I: Integer;
  F: TextFile;
begin
...
procedure TDebugFlatForm.ComboBoxCpChange(Sender: TObject);
begin
  case ComboBoxCp.ItemIndex of
    0:
      a.Cp := 1;
...
procedure TDebugFlatForm.ComboBoxD1Change(Sender: TObject);
begin
  if ComboBoxD1.ItemIndex = 0 then
    a.d1 := 0
  else
...
procedure TDebugFlatForm.ComboBoxD1KeyPress(Sender: TObject; var Key: Char);
begin
  if Key <> #13 then
    Key := #0;
end;

procedure TDebugFlatForm.ComboBoxSectChange(Sender: TObject);
begin
  a.SectionIndex := ComboBoxSect.ItemIndex;
  Refresh;
end;

procedure TDebugFlatForm.ComboBoxTensChange(Sender: TObject);
begin
  case ComboBoxTens.ItemIndex of
    0:
      a.Tension := ttAutomatic;
...
procedure TDebugFlatForm.EditN1Exit(Sender: TObject);
begin
  try
    a.N1 := StrToFloat(EditN1.Text);
  except
...
procedure TDebugFlatForm.EditP1Exit(Sender: TObject);
begin
  try
    a.P1 := StrToFloat(EditP1.Text);
  except
...
procedure TDebugFlatForm.EditUpExit(Sender: TObject);
begin
  try
    a.UP := StrToFloat(EditUp.Text);
  except
...
procedure TDebugFlatForm.ComboBoxAExit(Sender: TObject);
begin
  try
    if ComboBoxA.Text = 'Не важно' then
      a.a := 0
...
procedure TDebugFlatForm.FormShow(Sender: TObject);
var
  NB: TFlatInput;
begin
  ...
  a := TFlatBelt.Create(NB);

  Refresh;
end;

procedure TDebugFlatForm.Refresh;
var
